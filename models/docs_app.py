...
class DocsApp:
    def __init__(self):
        self.db_path = os.path.join(DB_DIR, 'documents.db')
        self.init_database()
    ...
    def list_documents(self, user_phone):
        ...
        try:
            ...
            # First, log all documents in the database for this user
            cursor.execute('''
    ...
    def retrieve_document(self, user_phone, query):
        ...
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute('SELECT id, drive_file_id, filename, description FROM documents WHERE user_phone = ?', (user_phone,))
            documents = cursor.fetchall()
