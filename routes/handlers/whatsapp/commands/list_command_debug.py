"""
List Command Handler (Debug Version)
---------------------------------
This module handles the 'list' command for WhatsApp users with detailed logging.
"""
...
class ListCommandHandler(BaseCommandHandler):
    ...
    async def handle(self, from_number):
        ...
        try:
            ...
            print(f"[DEBUG] {command_id} - Adding unique identifier to message")
            message = self.add_unique_identifier(message, "list", command_id)
            print(f"[DEBUG] {command_id} - Message with identifier: {message[:100]}...")
